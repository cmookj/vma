...
extern int dsyev_(char *, char *, integer *, doublereal *, integer *,
...
public:
  ...
  vec &operator+=(const vec &rhs) {
    std::transform(_elem.cbegin(), _elem.cend(), rhs.cbegin(), _elem.begin(),
                   std::plus<>{});
    return *this;
  }
...
template <size_t DIM>
bool operator< (const vec<DIM>& l, const vec<DIM>& r) {
...
public:
  ...
  mat &operator*=(const mat<DIM_COLS, DIM_COLS, T> &m) {
    ...
    if constexpr (std::is_same_v<T, double>)
      ...
      cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS,
                  DIM_COLS, 1., _elem.data(), DIM_ROWS, m.data(), DIM_COLS, 0.,
                  elm.data(), DIM_ROWS);
...
mat<DIM_ROWS, DIM_COLS, T> operator*(const mat<DIM_ROWS, DIM, T> &m1,
  ...
  if constexpr (std::is_same_v<T, double>)
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS,
                DIM, 1, m1.data(), DIM_ROWS, m2.data(), DIM, 0., elm.data(),
                DIM_ROWS);
  else if constexpr (std::is_same_v<T, complex_t>) {
    double alpha{1.0};
    double beta{0.};
    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS,
                DIM, &alpha, m1.data(), DIM_ROWS, m2.data(), DIM, &beta,
                elm.data(), DIM_ROWS);
  }
...
vec<DIM_COLS, T> operator*(const mat<DIM_ROWS, DIM_COLS, T> &m,
  ...
  if constexpr (std::is_same_v<T, double>)
    ...
    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, 1,
                DIM_COLS, &alpha, m.data(), DIM_ROWS, v.data(), DIM_COLS, &beta,
                elm.data(), DIM_COLS);
  }
...
vec<DIM_COLS, T> operator*(const vec<DIM_ROWS, T> &v,
  ...
  if constexpr (std::is_same_v<T, double>)
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 1, DIM_ROWS,
                DIM_COLS, 1, v.data(), 1, m.data(), DIM_ROWS, 0., elm.data(),
                1);
  else if constexpr (std::is_same_v<T, complex_t>) {
    double alpha{1.0};
    double beta{0.};
...
svd_t<DIM_ROWS, DIM_COLS> svd(const mat<DIM_ROWS, DIM_COLS> &M) {
  ...
  char jobz = 'A';
  integer_t lwork = 3 * ds * ds + std::max(std::max(m, n),
...
eigensystem<DIM> eigen(const mat<DIM, DIM> &M, eigen js = eigen::val) {
  ...
  if (is_symmetric(M)) {
    ...
    dgeev_(&JOBVL, &JOBVR, &N, A.data(), &LDA, WR.get(), WI.get(), VL.data(),
