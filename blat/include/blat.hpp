...
public:
    ...
    bool operator==(const vec& rhs) const {
        ...
        std::transform(
            cbegin(), cend(), rhs.cbegin(), diff.begin(), std::minus<> {}
        );
    ...
    vec& operator+=(const vec& rhs) {
        std::transform(
            _elem.cbegin(), _elem.cend(), rhs.cbegin(), _elem.begin(),
            std::plus<> {}
        );
        return *this;
    }
    ...
    vec& operator*=(const double& s) {
        std::transform(
            _elem.cbegin(), _elem.cend(), _elem.begin(),
...
bool collinear(const vec<DIM>& a, const vec<DIM>& b, double tol = TOL) {
    ...
    std::transform(
        a.cbegin(), a.cend(), b.cbegin(), ratio.begin(), std::divides<> {}
    );
    ...
    return std::adjacent_find(
               ratio.begin(), ratio.end(), nan_skipping_not_equal_to
           ) == ratio.end();
}
...
template <size_t DIM>
bool collinear(
    const vec<DIM, complex_t>& a, const vec<DIM, complex_t>& b, double tol = TOL
) {
    ...
    auto nan_skipping_not_equal_to =
        [&tol](const complex_t& a, const complex_t& b) {
...
public:
    ...
    mat& operator*=(const mat<DIM_COLS, DIM_COLS, T>& m) {
        ...
        if constexpr (std::is_same_v<T, double>)
            cblas_dgemm(
                CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS,
                DIM_COLS, 1., _elem.data(), DIM_ROWS, m.data(), DIM_COLS, 0.,
                elm.data(), DIM_ROWS
            );
        else if constexpr (std::is_same_v<T, complex_t>)
            cblas_zgemm(
...
std::string to_string(
    const mat<DIM_ROWS, DIM_COLS, T>& M, output_fmt fmt = output_fmt::nml
) {
    std::stringstream strm {};
 ...
 */
template <size_t DIM_ROWS, size_t DIM_COLS, typename T>
mat<DIM_ROWS, DIM_COLS, T> operator+(
    const mat<DIM_ROWS, DIM_COLS, T>& a, const mat<DIM_ROWS, DIM_COLS, T>& b
) {
    auto result {a};
    result += b;
    return result;
...
operator*(const mat<DIM_ROWS, DIM, T>& m1, const mat<DIM, DIM_COLS, T>& m2) {
    ...
    if constexpr (std::is_same_v<T, double>)
        cblas_dgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS, DIM,
            1, m1.data(), DIM_ROWS, m2.data(), DIM, 0., elm.data(), DIM_ROWS
        );
    else if constexpr (std::is_same_v<T, complex_t>) {
        double alpha {1.0};
        double beta {0.};
        cblas_zgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, DIM_COLS, DIM,
            &alpha, m1.data(), DIM_ROWS, m2.data(), DIM, &beta, elm.data(),
            DIM_ROWS
        );
    }
...
operator*(const mat<DIM_ROWS, DIM_COLS, T>& m, const vec<DIM_COLS, T>& v) {
    ...
    if constexpr (std::is_same_v<T, double>)
        cblas_dgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, 1, DIM_COLS, 1,
            m.data(), DIM_ROWS, v.data(), DIM_COLS, 0., elm.data(), DIM_COLS
        );
        ...
        double beta {0.};
        cblas_zgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, DIM_ROWS, 1, DIM_COLS,
            &alpha, m.data(), DIM_ROWS, v.data(), DIM_COLS, &beta, elm.data(),
            DIM_COLS
        );
    }
...
operator*(const vec<DIM_ROWS, T>& v, const mat<DIM_ROWS, DIM_COLS, T>& m) {
    ...
    if constexpr (std::is_same_v<T, double>)
        cblas_dgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, 1, DIM_ROWS, DIM_COLS, 1,
            v.data(), 1, m.data(), DIM_ROWS, 0., elm.data(), 1
        );
    else if constexpr (std::is_same_v<T, complex_t>) {
        double alpha {1.0};
        double beta {0.};
        cblas_zgemm(
            CblasColMajor, CblasNoTrans, CblasNoTrans, 1, DIM_ROWS, DIM_COLS,
            &alpha, v.data(), 1, m.data(), DIM_ROWS, &beta, elm.data(), 1
        );
    }
...
template <size_t DIM_ROWS, size_t DIM_COLS, typename T>
bool approx(
    const mat<DIM_ROWS, DIM_COLS, T>& M1, const mat<DIM_ROWS, DIM_COLS, T>& M2,
...
svd_t<DIM_ROWS, DIM_COLS> svd(const mat<DIM_ROWS, DIM_COLS>& M) {
    ...
    integer_t lwork =
        3 * ds * ds + std::max(
                          std::max(m, n), 5 * std::min(m, n) * std::min(m, n) +
                                              4 * std::min(m, n)
                      );
    ...
    dgesdd_(
        &jobz, &m, &n, el.get(), &lda, s.get(), u.get(), &ldu, vt.get(), &ldvt,
...
eigensystem<DIM> eigen(const mat<DIM, DIM>& M, eigen js = eigen::val) {
    ...
    if (is_symmetric(M)) {
        ...
        dsyev_(
            &JOBZ, &UPLO, &N, A.data(), &LDA, W.get(), WORK.get(), &LWORK, &INFO
        );
        ...
        dgeev_(
            &JOBVL, &JOBVR, &N, A.data(), &LDA, WR.get(), WI.get(), VL.data(),
            &LDVL, VR.data(), &LDVR, WORK.get(), &LWORK, &INFO
        );
            ...
            for (size_t j = 1; j <= UN; ++j) {
                ...
                case eigen::vec:
                    if (WI[j - 1] != 0. && j < UN && WI[j - 1] == -WI[j]) {
                        ...
                        // Right Eigenvectors
                        es.eigvecs_rt.set_col(
                            j, cvec(VR.col(j), VR.col(j + 1))
                        );
                        es.eigvecs_rt.set_col(
                            j + 1, cvec(VR.col(j), -VR.col(j + 1))
                        );

                        // Left Eigenvectors
                        es.eigvecs_lft.set_col(
                            j, cvec(VL.col(j), VL.col(j + 1))
                        );
                        es.eigvecs_lft.set_col(
                            j + 1, cvec(VL.col(j), -VL.col(j + 1))
                        );
